...
Returns: A random sample of 50 recipes to keep the payload size manageable.
'''
...
def get_recipes():
    ...
    return jsonify(df.sample(50, random_state=42).to_dict(orient='records'))
...
def get_clusters():
    ...
    return jsonify(df[['Recipe_name','Diet_type','cluster']].sample(50, random_state=42).to_dict(orient='records'))
